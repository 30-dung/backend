...
public class AuthController {
    ...
    public ResponseEntity<?> forgotPassword(@Valid @RequestBody ForgotPasswordRequest request) {
        try {
            ...
            return ResponseEntity.ok(new AuthResponse(null, "Mã OTP đã được gửi đến email của bạn"));
        } catch (MessagingException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new AuthResponse(null, "Không thể gửi OTP: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> resetPassword(@Valid @RequestBody ResetPasswordRequest request) {
        try {
            ...
            return ResponseEntity.ok(new AuthResponse(null, "Đặt lại mật khẩu thành công"));
